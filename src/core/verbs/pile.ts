...
// Writing the Pile, which is **one verb and its undoing**.
//
// There is nothing else to write here, and that is the design rather than a stage it has
// not reached: the Pile composes itself from the Wants, the unread stops of the
...
// proposal built by `queries/pile.ts` and nothing more; opening it is
 ...
 * about.
 */
export async function pinToPile(subject: PinnedSubject): Promise<void> {
  ...
  await refusing(
    () =>
      ...
      subject.kind === "story"
        ? query(
            `insert into pile_pin (story_id) values ($1)
             on conflict (story_id) where story_id is not null
             do update set pinned_at = now()`,
            [subject.id]
          )
        : query(
            `insert into pile_pin (series_id, series_position) values ($1, $2)
    ...
    (constraint) => {
      switch (constraint) {
        case "pile_pin_story_exists":
          return NO_SUCH_STORY;
        case "pile_pin_series_exists":
          return NO_SUCH_SERIES;
        default:
          return "That could not be pinned to the Pile.";
      }
    }
  );
}
 ...
 */
export async function unpinFromPile(subject: PinnedSubject): Promise<void> {
  ...
  const unpinned =
    subject.kind === "story"
      ? await query<{ gone: boolean }>(
          "delete from pile_pin where story_id = $1 returning true as gone",
          [subject.id]
        )
      : await query<{ gone: boolean }>(
          `delete from pile_pin
