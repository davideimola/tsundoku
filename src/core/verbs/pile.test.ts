...
import { pinToPile, unpinFromPile } from "./pile.ts";
import { declareSeries } from "./series.ts";
import { createStory } from "./story.ts";

// Seam 1, the write side — and there is only one thing to write here. The Pile
...
> {
  return query("select story_id, series_id, series_position from pile_pin order by pinned_at");
}
...
describe("pinning", () => {
  ...
  it("names a position of a Series, because the shopping half is an object and not a line", async () => {
    ...
    await pinToPile({ kind: "series", id: seriesId, position: 2 });
  ...
  it("keeps two positions of one Series apart: they are two things to buy", async () => {
    ...
    await pinToPile({ kind: "series", id: seriesId, position: 2 });
    await pinToPile({ kind: "series", id: seriesId, position: 3 });
  ...
  it("is refused where there is no such Story or Series", async () => {
    await expect(
      pinToPile({ kind: "story", id: "00000000-0000-0000-0000-000000000000" })
    ).rejects.toMatchObject({ code: "not-found" });

    // A URL and an assistant can both carry anything. Prose, not a 500.
    await expect(pinToPile({ kind: "series", id: "banana", position: 1 })).rejects.toSatisfy(
      isRefusal
    );
  });
  ...
  it("is refused where the position is not a place in a line", async () => {
    ...
    await expect(pinToPile({ kind: "series", id: seriesId, position: 0 })).rejects.toMatchObject({
      code: "invalid",
    });
    await expect(
      pinToPile({ kind: "series", id: seriesId, position: Number.NaN })
  ...
  it("is refused by Postgres, not by an `if`, when a row would name two subjects at once", async () => {
    ...
    await expect(
      query("insert into pile_pin (story_id, series_id, series_position) values ($1, $2, 1)", [
        storyId,
        seriesId,
      ])
    ).rejects.toMatchObject({ constraint: "pile_pin_has_one_subject" });

    await expect(
      query("insert into pile_pin (series_id) values ($1)", [seriesId])
    ).rejects.toMatchObject({ constraint: "pile_pin_a_series_pin_names_a_position" });
  });
});
...
describe("unpinning", () => {
  ...
  it("lifts one position of a Series and leaves the other pinned", async () => {
    ...
    await pinToPile({ kind: "series", id: seriesId, position: 2 });
    await pinToPile({ kind: "series", id: seriesId, position: 3 });
    await unpinFromPile({ kind: "series", id: seriesId, position: 2 });
  ...
  it("is refused on something that was not pinned, rather than passing silently", async () => {
    ...
    await expect(unpinFromPile({ kind: "story", id: storyId })).rejects.toMatchObject({
      code: "not-found",
    });
  });
});
