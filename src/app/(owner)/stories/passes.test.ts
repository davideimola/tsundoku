...
import type { StoryPass } from "@/core/queries/story";
...
// replaced. It holds the **words** a Pass is said in and the one predicate the acts on the
...
// ended* is what leaves a Story open, it is what the dashboard's top band is, and it is what
...
/** A Pass, with only the fields the wording is derived from said out loud. */
function pass(said: Partial<StoryPass> = {}): StoryPass {
  return {
    id: "a-pass",
    medium: "paper",
    outcome: "finished",
...
describe("when a Pass happened", () => {
  it("is the span, where both ends are known", () => {
    expect(whenItHappened(pass({ startedOn: "2021-05-01", endedOn: "2021-06-01" }))).toBe(
  ...
  it("is one end, where only one is known", () => {
    expect(whenItHappened(pass({ startedOn: "2021-05-01" }))).toBe("from 2021-05-01");
    expect(whenItHappened(pass({ endedOn: "2021-06-01" }))).toBe("until 2021-06-01");
  });

  // The tense is the whole difference: a Pass that is still open has no end to name, so
  // *since* is what its start means. *From* would say it is over.
  it("is `since` while the Pass is still open", () => {
    expect(whenItHappened(pass({ outcome: null, startedOn: "2026-08-01" }))).toBe(
      "since 2026-08-01"
    );
  });

  it("says so plainly where no date was ever recorded", () => {
    expect(whenItHappened(pass())).toBe("no date recorded");
    expect(whenItHappened(pass({ outcome: null }))).toBe("open, no date recorded");
  });
});

describe("how a Pass went", () => {
  it("is the medium and the outcome", () => {
    expect(howItWent(pass({ medium: "digital", outcome: "abandoned" }))).toBe("digital, abandoned");
  });

  // Not an absent outcome dressed up as one: a Pass with none has not ended, and the
  // words say the act rather than a missing field. *Under way* rather than *reading*,
  // because half of what a pass goes through is played (#58).
  it("says still under way where there is no outcome yet", () => {
    expect(howItWent(pass({ outcome: null }))).toBe("paper, still under way");
  });
});

describe("the Pass that is open", () => {
  it("is the one that has started and not ended", () => {
    expect(stillOpen(pass({ outcome: null }))).toBe(true);
    expect(stillOpen(pass({ outcome: "finished" }))).toBe(false);
    expect(stillOpen(pass({ outcome: "abandoned" }))).toBe(false);
  });

  // The stack is newest first, and a second pass is the ordinary case here: the open one is
  // found among the settled ones rather than assumed to lead.
  it("is found in a stack that has settled Passes above it", () => {
    const open = pass({ id: "the-second-pass", outcome: null });

    expect(theOpenPass([pass({ id: "finished-in-2021" }), open])?.id).toBe("the-second-pass");
  });

  // Which is what makes the Story read `to read`, `read` or `abandoned` rather than
  // still open, and what puts *start a Pass* in the hero instead of *finish it*.
  it("is nothing at all where every Pass has ended", () => {
    expect(theOpenPass([pass(), pass({ outcome: "abandoned" })])).toBeUndefined();
    expect(theOpenPass([])).toBeUndefined();
  });
});
...
describe("the Pass in the owner's hands", () => {
  it("says since when, where a day was recorded", () => {
    expect(passNow(pass({ outcome: null, startedOn: "2026-08-01" }))).toBe(
      "Under way since 2026-08-01."
    );
  });

  it("says only that it is open, where none was", () => {
    expect(passNow(pass({ outcome: null }))).toBe("Under way now.");
  });
});
